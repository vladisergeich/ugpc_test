<template>
        <div class="row">
                ...
                <div class="info-container">
                    ...
                    <div class="info-container_body">
                        ...
                        <template v-if="(selectedSecondaryOperation != null) && (selectConsumpShaft == null) && (shiftReceptionOperation == null)">
                            ...
                            <a-button class="operation_test_print_timing" @click="showModalTimingOperation">Выставить время вручную</a-button>
                        ...
                        <template v-if="(selectConsumpShaft!=null) && (this.shiftReceptionOperation == null)">
                            <TestPrintOperationInfoContainer :selectConsumpShaft="selectConsumpShaft" :selectMainOperation="selectMainOperation" :selectedMainOperation="selectedMainOperation" :deleteConsumpShaft="deleteConsumpShaft" :closeOperation="closeOperation" :startOperation="startOperation" :deleteOperation="deleteOperation"/>
                        </template>
                    </div>
                </div>
            ...
            <div v-if="this.selectMachine != ''" class="col-lg-4">
                <WarehouseShaftsContainer :shiftReceptionOperation="shiftReceptionOperation" :consumpShaft="consumpShaft" :shafts="selectShaftForTestPrint"/>
            </div>
        </div>
</template>
...
import TestPrintOperationInfoContainer from "../Components/TestPrintOperationInfoContainer.vue";
...
export default {
        components: {
            ...
            ShaftInfoContainer,
            TestPrintOperationInfoContainer,
        },

        props: {
            secondary_operations: Array,
            operation_test_print: Array,
        ...
        created() {
            window.Echo.private('shaft-selected').listen('LotConsumpTestPrintMachine', (e) => {
                this.selectConsumpShaft = e;
            });
        },

        mounted () {
            this.startWorkShiftCard();
        },
        data() {
            return {
                columns_operations: [
        ...
        methods: {
            ...
            startWorkShiftCard(){
                if (this.selectMachine != '') {
                    ...
                    .then(response => {
                        this.selectShaftForTestPrint = response.data[0];
            ...
            startOperation() {
                ...
                this.postOperation(this.urlstartoperation, 'Пробная печать', null, () => {
                    this.selectedMainOperation = 'Пробная печать';
                });
            },
            ...
            closeOperation(operation) {
                ...
                if (this.selectedMainOperation != null){
                    inputsFilled = [this.selectConsumpShaft.test_print_roughness_chrome, this.selectConsumpShaft.test_print_hardness_chrome].every(input => !!input);
                    this.selectConsumpShaft.test_print = true;
                } else {
                    inputsFilled = true;
                }
                    ...
                    onOk: () => {
                        axios.post(this.urlcloseoperation, {
                            ...
                            .then(response => {
                                ...
                                this.selectShaftForTestPrint = response.data[4];
            ...
            consumpShaft(shaft) {
                if (this.selectConsumpShaft != null) {
                    ...
                    .then(response => {
                        ...
                        this.selectShaftForTestPrint = response.data[2];
                    })
                    .catch(error => {
                        alert("Ошибка! Обратитесь поддержку");
                    });
...
.base_test_print_make {
    display: flex;
    margin-right: 20px;
}
.test_print_edition_make {
    display: flex;
    margin-right: 20px;
}
...
.test_print_edition_block {
    margin-right: 20px;
}


.base_test_print_block {
    margin-right: 20px;
}

.shaft_info_row_test_print_edition {
    display: flex;
}

.shaft_info_row_base_test_print {
    display: flex;
}
</style>
