<template>
    <div class="main_operation_wrapper">   
        <div class="shaft_info_row" v-if="this.selectedMainOperation != null">
            <div class="shaft_info_test_print_block">
                <span class="block_title">Шероховатость хром:</span>
                <a-input class="block_test_print_value" v-model="selectConsumpShaft.test_print_roughness_chrome" />
            </div>
            <div class="shaft_info_test_print_block">
                <span class="block_title">Твердость хром:</span>
                <a-input class="block_test_print_value" v-model="selectConsumpShaft.test_print_hardness_chrome"/>
            </div>
        </div>
        <div class="shaft_info_row_btn">
            <div class="current_operation_info" v-if="this.selectedMainOperation != null">
                ...
                <a-modal
                    ...
                    <template slot="footer">
                        ...
                        <a-button key="delete_operation" type="primary" @click="deleteOperation(selectedMainOperation)">
                        Удалить
                        </a-button>
                    </template>
            ...
            <a-button class="operation_test_print_btn" style="margin-right: 10px;" v-if="this.selectedMainOperation == null" @click="startOperation(selectMainOperation)">Печать</a-button>
            <a-button v-if="this.selectedMainOperation != null" class="operation_engraver_btn_close" @click="closeOperation(selectedMainOperation)">Завершить</a-button>
            <a-button v-if="this.selectedMainOperation == null" class="operation_test_print_btn" @click="deleteConsumpShaft(selectConsumpShaft)">Отменить</a-button>
        </div>
    </div>
</template>
    ...
    methods: {
        callcloseOperation() {
            if (this.closeOperation) {
                this.closeOperation(selectedMainOperation);
            }
        },

        callcloseDeleteConsumpShaft() {
            if (this.deleteConsumpShaft) {
                this.deleteConsumpShaft(selectConsumpShaft);
            }
        },

        callstartOperation() {
            if (this.startOperation) {
                this.startOperation(selectedMainOperation);
            }
        },
...
.operation_test_print_btn_close {
    display: flex;
    padding: 8px 16px;
    justify-content: center;
    align-items: center;
