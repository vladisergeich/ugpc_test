                    ...
                    <div class="row" style="text-align: right;">
                        <div class="col-md-12">
                            ...
                            <b-modal id="ShaftWarehouseModal" title="Подбор по валам">
                                <div class="div" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px; gap: 20px;">
                                    <a-input-search 
                                        :allow-clear="true"
                                        v-model="searchText"
                                    />
                                    <button @click="allAddShafts" v-if="showAllAddBtn" class="all_add_btn">Добавить</button>
                                </div>
                                <a-table
                        ...
                        <table class="table table-hover">
                            <thead>
                                <tr>
                                    ...
                                    <th scope="col" v-if="selectShaftRequest.sender == 'БиекТау' || selectShaftRequest.sender == 'ИП Калмыкова'">Пред. ОКВИД</th>
                            ...
                            <tbody>
                                <tr v-for="(shaft, index) in this.selectShaftRequestComposition" :key="index">
                                    ...
                                    <td ><span v-if="(selectShaftRequest.sender == 'БиекТау' || selectShaftRequest.sender == 'ИП Калмыкова') && shaft.shaft.last_okvid">{{ shaft.shaft.last_okvid.okvid_number ?? '' }}</span></td>
                                    <td >
        ...
        computed: {
            ...
            filteredShafts() {
                ...
                return this.selectShaftWarehouse.filter(shaft =>
                    ...
                    (shaft?.okvid?.okvid_number && shaft?.okvid?.okvid_number.toString().toLowerCase().startsWith(searchTextLower))
                );
            }
        ...
        methods: {
            ...
            onSelectChange(selectedRowKeys, selectedRows) {
                ...
                if (selectedRowKeys.length > 0) {
                    this.showAllAddBtn = true;
                } else {
                    this.showAllAddBtn = false;
                }
            },

            allAddShafts() {
                this.selectedRows.forEach((record, index) => {
                    this.AddRequestShaft(record);
                })
